...
class ListBrand extends Component
{
    ...
    public function deleteListCheckbox()
    {
        ...
        if(count($list_has_product) > 0){
            ...
            $this->dispatch('error', ['error' => 'Thể Loại <b>'.$list_has_product.'</b> đã có Truyện, vì vậy không thể xóa.']);
        }
        $this->render();
    }
    ...
    public function deleteBrand($id){
        ...
        if($checkProduct != null){
            $this->dispatch('error', ['error' => 'Thể Loại <b>'.$brand->name.'</b> đã có Truyện, vì vậy không thể xóa.']);
            return;
        }else{
            $brand = Brand::find($id);
            $brand->delete();
