                ...
                @foreach ($brands as $index => $brand)
                    <tr>
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            <input type="checkbox" name="cbx_delete_brand" {{$brand->hasProduct() ? 'disabled' : ''}} class="cursor-pointer w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" wire:model="selected_index.{{$index}}" value="{{$brand->id}}">
                        </td>
                        <td class="px-2 py-2 whitespace-nowrap text-center">{{ $brands->perPage() * ($brands->currentPage() - 1) + $loop->iteration }}</td>
                        <td class="px-2 py-2 whitespace-nowrap text-center">
                            @if ($brand->logo)
                                <img src="{{ asset('storage/images/brands/' . $brand->logo) }}" alt="Brand Logo" class="w-15 h-15 shadow-md">
                            @else
                                <img src="{{ asset('library/images/image-not-found.jpg') }}" alt="Brand Logo" class="w-15 h-15 shadow-md">
                            @endif
                        </td>
                        <td class="px-2 py-2 whitespace-nowrap">{{$brand->name}}</td>
                        <td class="px-2 py-2 whitespace-nowrap">{{$brand->slug}}</td>
                        <td class="px-2 py-2 whitespace-nowrap" title="{{$brand->description}}">
